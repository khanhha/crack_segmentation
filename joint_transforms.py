...
class JointRandomSizedCrop(object):
    ...
    def __init__(self, size, interpolation=Image.BILINEAR, area_scales = (0.7, 1.0)):
        self.size = size
        self.interpolation = interpolation
        self.area_scales = area_scales

    def __call__(self, imgs):
        for attempt in range(10):
            area = imgs[0].size[0] * imgs[0].size[1]
            target_area = random.uniform(self.area_scales[0], self.area_scales[1]) * area
